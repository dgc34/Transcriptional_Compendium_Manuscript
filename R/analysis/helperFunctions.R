...
plotDatSignatureTimeSeries <- function(signature, 
                                       data_list, 
                                       performance_metric = 'AUROC', 
                                       method = 'zScore', 
                                       number_permutations = 1000, 
                                       specific_method = 'geomMean', 
                                       pval_calc = F, 
                                       pval_perm = 1000, 
                                       parallel = F){
  debug_plot <- F
  
  class_nums <- sapply(data_list, function(x){length(table(x$pheno$Class))}) # number of classes per study
  if(min(class_nums == 1)){
    print(names(data_list)[which.min(class_nums)])
    stop('not all studies have two classes')
  }
  
  plot_dat <- plotDatBaseDf(data_list, signature)
  
  output_df_list <- list()
  for(i in 1:length(data_list)){
    current_study <- data_list[[i]]
    
    # reset between studies
    full_plot_dat <- NULL
    min_plot_dat <- NULL
    max_plot_dat <- NULL
    
    # data frame of SubjectID, score, Time.Point
    scores = calculateScoreRobust(filterObject = signature, datasetObject = current_study)
    full_plot_dat <- data.frame('SubjectID' = current_study$pheno$SubjectID,
                                'Score' = scores,
                                'Time.Point' = current_study$pheno$Time.Point,
                                'Class' = current_study$pheno$Class,
                                'class_binary' = current_study$class,
                                'Study' = current_study$formattedName,
                                stringsAsFactors = F)
    
    # if subjectID is missing, make unique values
    full_plot_dat <- full_plot_dat %>%
      mutate(position_in_category = 1:n()) %>%
      mutate(SubjectID = ifelse(is.na(SubjectID), paste0(Class, '_', position_in_category), SubjectID))
    
    # identify min and max classes 
    #max_class <- unique(current_study$pheno$Class[current_study$class == 1])
    #min_class <- unique(current_study$pheno$Class[current_study$class == 0])
    
    # split full df into min and max using class labels
    # we want the most discrimination, so min(negative class scores) vs. max(positive class scores)
    min_plot_dat <- full_plot_dat %>%
      filter(class_binary == 0) %>% # negative class
      group_by(SubjectID) %>%
      filter(Score == min(Score)) %>% # take the minimum per subject
      ungroup()
    
    max_plot_dat <- full_plot_dat %>%
      filter(class_binary == 1) %>% #positive class 
      group_by(SubjectID) %>%
      filter(Score == max(Score)) %>% #take the maximum per subject
      ungroup()
    
    # debug
    #print(table(full_plot_dat$Class, full_plot_dat$class_binary))
    
    
    # merge min and max to calculate AUC
    output_plot_dat <- list(max_plot_dat, min_plot_dat) %>%
      bind_rows() %>%
      distinct() 
    
    AUC = auroc(bool = output_plot_dat$class_binary, 
                score = output_plot_dat$Score)
    study_dat <- data.frame('Study' = current_study$formattedName,
                            'Scores' = AUC)
    
    if(pval_calc){
      if(length(unique(output_plot_dat$class_binary)) != 2){
        print(current_study$formattedName)
        print(signatures$filterDescription)
      }
      pval <- calculatePval(labels = output_plot_dat$class_binary, values = output_plot_dat$Score, true_value = AUC)
      study_dat$P.Value = pval
    }
    
    if(debug_plot){
      
      # add color to min and max plot dats, left join back into full plot dat
      tmp_color_dat <- bind_rows(min_plot_dat, max_plot_dat) %>%
        mutate('Color' = T)
      full_plot_dat <- full_plot_dat %>%
        left_join(tmp_color_dat) %>%
        mutate(Color = ifelse(is.na(Color), F, Color))
      #group_by(SubjectID) %>%
      #mutate(Color = ifelse((class_binary == 0 & Score == min(Score))|(class_binary == 1 & Score == max(Score)), 
      #                      F, T))
      
      debug_grob <- ggplot(full_plot_dat, aes(x = Time.Point, y = Score)) + 
        geom_point(aes(color = Color)) + 
        geom_line(aes(group = SubjectID)) + 
        facet_grid(. ~ Class, scales = 'free', space = 'free') + 
        labs(title = paste0('AUC = ', round(AUC,3), ' | p-val: ', round(pval, 3)))
      ggsave(debug_grob, filename = paste0('~/Dropbox/ts_debug/', signature$filterDescription, '_', unique(full_plot_dat$Study), '.png'))
    }
    
    output_df_list[[i]] <- study_dat
  }
  
  score_df <- output_df_list %>%
    bind_rows() 
  validation_ix <- sapply(plot_dat$Accession, function(x){
    if(x %in% signature$validation){
      return(T)
  ...
  plot_dat[discovery_ix, 'Type'] <- 'Discovery'
  plot_dat[validation_ix, 'Type'] <- 'Validation'
...
plotDatBaseDf <- function(data_list, signature){
  studies <- sapply(data_list, function(x){return(x$formattedName)})
  plot_dat <- data.frame('Study' = studies, 
                         'Accession' = unlist(lapply(studies, parseAccession)), 
                         'Platform' = unlist(lapply(studies, parsePlatform)), 
...
plotDatSignatureTimeSeriesStudyLevel <- function(signature, 
                                                 data_list, 
                                                 performance_metric = 'AUROC', 
                                                 method = 'zScore', 
                                                 number_permutations = 1000, 
  ...
  for(i in 1:length(data_list)){
    ...
    # compute AUCs for each time point where at least 50% of subjects were measured
    
    full_plot_dat <- full_plot_dat %>%
      mutate(Time.Point = ifelse(is.na(Time.Point), 'NA', Time.Point))
    
    # find time points with at least half of subjects measured — some studies have disparate points, aim for 50% of the max ever measured
    max_subjects <- full_plot_dat %>%  
      group_by(Time.Point, class_binary) %>%
      mutate('N' = length(unique(SubjectID))) %>% # get the number of unique subjects per time/class
      ungroup() %>%
      group_by(class_binary) %>%
      mutate('max_N' = max(N)) %>% # get the max number of subjects per class at any time
      ungroup() %>%
      group_by(Time.Point, class_binary, N, max_N) %>%
      summarize('frac_N' = N/max_N) %>% # calculate the fraction of the max for each time point
      ungroup() %>%
      distinct()
    
    
    healthy_times <- max_subjects %>%
      filter(frac_N >= 0.5, class_binary == 0) %>%
      pull(Time.Point)
    
    other_times <- max_subjects %>%
      filter(frac_N >= 0.5, class_binary == 1) %>%
      pull(Time.Point)
    
    output_plot_dat <- NULL
    best_0_time <- NA
    best_1_time <- NA
    max_auc <- 0
    for (htime in healthy_times){
      for (otime in other_times){
        min_plot_dat = full_plot_dat %>%
          filter(Time.Point == htime, class_binary == 0)
        max_plot_dat = full_plot_dat %>%
          filter(Time.Point == otime, class_binary == 1)
        
        auc_plot_dat <- bind_rows(min_plot_dat, max_plot_dat)
        current_auc = auroc(bool = auc_plot_dat$class_binary,
                            score = auc_plot_dat$Score)
        #print(current_auc)
        if(current_auc > max_auc){
          max_auc <- current_auc
          best_0_time <- htime
          best_1_time <- otime
          output_plot_dat <- bind_rows(min_plot_dat, max_plot_dat) %>%
            distinct()
        }
      }
    }
    ...
    if(debug_plot){
      ...
      debug_grob <- ggplot(full_plot_dat, aes(x = Time.Point, y = Score)) +
        geom_boxplot(outlier.shape = NA, aes(group = Time.Point)) +
        geom_point(aes(color = Color)) + 
        geom_line(aes(group = SubjectID)) + 
        facet_grid(. ~ Class, scales = 'free', space = 'fixed') + 
        labs(title = paste0('AUC = ', round(AUC,3), ' | p-val: ', round(pval, 3)))
      ggsave(debug_grob, filename = paste0('~/Dropbox/ts_debug/', signature$filterDescription, '_', unique(full_plot_dat$Study), 'sdy_boxplot.png'))
    }
...
plotDatSignatureTimeSeriesAltPval <- function(signature, 
                                              data_list, 
                                              performance_metric = 'AUROC', 
                                              method = 'zScore', 
                                              number_permutations = 1000, 
  ...
  for(i in 1:length(data_list)){
    ...
    if(pval_calc){
      ...
      pval <- calculatePvalTS(ts_df = full_plot_dat, n_perm = pval_perm, AUC = AUC)
      study_dat$P.Value = pval
    }
...
# give the ts_df with Time.Point / Subject / Scores
# AUC with the observed AUC
# n_perm
calculatePvalTS <- function(ts_df, AUC, n_perm = 1000){
  auc_vect <- vector(mode = 'double', length = n_perm)
  # for each iteration
  for (iteration in 1:n_perm){
    # permute class labels within subject
    tmp <- ts_df %>%
      group_by(SubjectID) %>%
      mutate('ix' = sample(size = n(), x = 1:n(), replace = F)) %>% # shuffle the indices
      mutate(m = sum(class_binary == 0))  # find out how many healthy time points there are per subject
    tmp2 <- tmp %>%
      arrange(desc(ix)) %>%
      mutate('new_row' = 1:n()) %>%
      mutate(class_binary = ifelse(new_row <= m, 0, 1)) # if there are m healthy times before permutation, set the first m rows to be healthy
    head(tmp2)
    tmp2 %>% filter(grepl(pattern = 'subject 3$', x = SubjectID)) 
    tmp2 %>% filter(grepl(pattern = 'subject 6$', x = SubjectID))
    ...
    # compute AUC from max/min values
    auc_plot_dat <- bind_rows(min_plot_dat, max_plot_dat)
    current_auc = auroc(bool = auc_plot_dat$class_binary,
                        score = auc_plot_dat$Score)
    ...
    if(F){
      # plot 
      used_values <- bind_rows(min_plot_dat, max_plot_dat) %>%
        mutate('color_binary' = ifelse(class_binary == 0, 0, 1), 'used' = T)
      full_values <- tmp2 %>% left_join(used_values) %>%
        mutate('used' = ifelse(is.na(used), F, T))
      p0 <- ggplot(full_values, aes(x = Time.Point, y = Score, group = SubjectID)) + 
        geom_point(aes(shape = as.factor(class_binary), color = as.factor(used))) + 
        geom_line() + 
        #facet_grid(. ~ Class, scales = 'free', space = 'free') + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
        labs(title = paste0('AUC = ', round(current_auc, 3)))
      p0a <- ggplot(full_values %>% filter(used), aes(x = Time.Point, y = Score, group = SubjectID)) + 
        geom_point(aes(shape = as.factor(class_binary), color = as.factor(used))) + 
        geom_line() + 
        #facet_grid(. ~ Class, scales = 'free', space = 'free') + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
        labs(title = paste0('AUC = ', round(current_auc, 3)))
      p1 <- ggplot(full_values %>% filter(used), aes(x = as.factor(class_binary), y = Score)) + 
        geom_boxplot()
      p0 + p0a + p1
    }
    auc_vect[iteration] <- current_auc
  }
  
  if(F){
    hist(auc_vect); abline(v = AUC, col = 'red')
  }
  
  # compute p-value against observed AUC
  p_val <- mean(auc_vect >= AUC)
}


scoreSignature <- function(current_sig, n){
  flu_train_summarized_score <- calculateWeightedAUC(flu_train, N_flu_train, current_sig)
  other_train_summarized_score <- calculateWeightedAUC(other_train, N_other_train, current_sig)
  flu_test_summarized_score <- calculateWeightedAUC(flu_test, N_flu_test, current_sig)
  other_test_summarized_score <- calculateWeightedAUC(other_test, N_other_test, current_sig)
  
  #other_train_FPR <-  sapply(other_train, virtualFPR, sig = current_sig, fraction = F)
  #other_rest_FPR <- sapply(other_train, virtualFPR, sig = current_sig, fraction = F)
  #other_train_summarized_FPR = sum(other_train_FPR, na.rm = T) / sum(Denom_other_train)
  #other_test_summarized_FPR <- sum(sapply(other_test, virtualFPR, sig = current_sig, fraction = T)) / sum(Denom_other_test)
  
  other_train_summarized_FPR <- calculateWeightedFPR(other_train, Denom_other_train, current_sig)
  other_test_summarized_FPR <- calculateWeightedFPR(other_test, Denom_other_test, current_sig)
  
  
  results <- tryCatch({
    data.frame('posGenes' = ifelse(length(current_sig$posGeneNames) > 0, paste(current_sig$posGeneNames, collapse = ';'), ''),
               'negGenes' = ifelse(length(current_sig$negGeneNames) > 0, paste(current_sig$negGeneNames, collapse = ';'), ''),
               'k' = n, 
               'flu_train' = flu_train_summarized_score,
               'other_train' = other_train_summarized_score,
               'flu_test' = flu_test_summarized_score,
               'other_test' = other_test_summarized_score,
               'fpr_train' = other_train_summarized_FPR,
               'fpr_test' = other_test_summarized_FPR,
               stringsAsFactors = F)
  }, error = function(w){
    saveRDS(object = current_sig, '~/Dropbox/current_sig.RDS')
    print(w)
  })
  return(results)
}

calculateWeightedAUC <- function(data_list, N_list, sig){
  AUCs <- sapply(data_list, 
                 calculateAUROC, 
                 signature = sig, 
                 suppressMessages = T,
                 method = 'geomMean')
  na_ix <- is.na(AUCs)
  summarized_AUC <- sum(N_list[!na_ix] * AUCs[!na_ix] / sum(N_list[!na_ix]), na.rm = T)
  return(summarized_AUC)
}

calculateWeightedFPR <- function(data_list, N_list, sig){
  FPRs <- sapply(data_list, 
                 virtualFPR, 
                 sig = sig,
                 fraction = T, 
  )
  ...
  if(sum(na_ix) == 1){
    ...
    summarized_FPR <- sum(N_list[!na_ix] * FPRs[!na_ix], na.rm = T)/sum(N_list[!na_ix], na.rm = T)
    return(summarized_FPR)
  }
}

generateNlist <- function(data_list){
  return(sapply(data_list, function(x){min(table(x$pheno$Class))}))
}

generateDenomlist <- function(data_list, positive_class = 'Virus'){
  return(sapply(data_list, function(x){return(sum(x$pheno$Class == positive_class))}))
}