...
disc_df %>% group_by(Signature, Comparison) %>% summarize('med' = median(Scores)) %>% ungroup() %>% group_by(Comparison) %>% summarize('min' = min(med))

date <- str_extract(string = input_file, pattern = '\\d+')
saveRDS(pout, file = paste0('~/Dropbox/ggplot_objects/3b_discovery_AUROCs_', date, '_', contrast, '.RDS'))
